...
from model.MDGCRN import MDGCRN as Network
...
args.add_argument('--model', default='MDGCRN', type=str)
...
if args.mode == 'train':
    ...
    # 在没有CUDA的情况下使用CPU加载模型
    model.load_state_dict(torch.load('./pre-trained/{}.pth'.format(args.dataset), map_location=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')))
    print("Load saved model")
    trainer.test(model, trainer.args, test_loader, scaler, trainer.logger)
else:
    raise ValueError