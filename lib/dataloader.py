...
from lib.normalization import NScaler, MinMax01Scaler, MinMax11Scaler, StandardScaler, ColumnMinMax01Scaler
...
def normalize_dataset(data, normalizer, column_wise=False):
    if normalizer == 'max01':
        ...
        scaler = ColumnMinMax01Scaler(data.min(axis=0), data.max(axis=0))
        data = scaler.transform(data)
...
def data_loader(X, Y, batch_size, shuffle=True, drop_last=True):
    ...
    # TensorDataset 用于将输入数据和目标数据进行打包，使它们能够以数据对的形式一起被访问
